...
def create_data_sets(output_dir=OUTPUT_DIR):
    ...
    val_dat = \
    val_uniq_data[val_uniq_data['metric'] == 'validity'][['value', 'iteration']].set_index('iteration').sort_index()[
        'value']
    unq_dat = \
    val_uniq_data[val_uniq_data['metric'] == 'unique'][['value', 'iteration']].set_index('iteration').sort_index()[
        'value']
    nearest_neighbor_data = data_agg[data_agg['metric'] == 'nearest neighbor correlation'].groupby('iteration')[
        'value'].mean().sort_index()

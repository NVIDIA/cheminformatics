...
class ExCAPEDataset():
    def __init__(self):
        ...
        data_dir = '/data'
        self.raw_data_path = raw_data_path = os.path.join(data_dir, 'raw_data.csv')
...
class ExCAPEBioactivity(ExCAPEDataset):
    def __init__(self, data_dir='/data/ExCAPE', **kwargs):
        super().__init__()
        self.name = 'ExCAPE Bioactivity'
        self.table_name = 'excape_activity'
        self.raw_data_path = raw_data_path = os.path.join(data_dir, 'pubchem.chembl.dataset4publication_inchi_smiles_v2.tsv')
        self.filter_data_path = os.path.join(data_dir, 'ExCAPE_filtered_data.csv')

    def load(self, columns=['SMILES', 'Gene_Symbol'], filter_len=None):
...
class ExCAPEFingerprints(ExCAPEDataset):
    def __init__(self, data_dir='/data/ExCAPE', **kwargs):
        super().__init__()
        self.name = 'ExCAPE Fingerprints'
        self.table_name = 'excape_fp'
        self.raw_data_path = raw_data_path = os.path.join(data_dir, 'pubchem.chembl.dataset4publication_inchi_smiles_v2.tsv')
        self.filter_data_path = os.path.join(data_dir, 'ExCAPE_filtered_data.csv')

...
class MegaMolBART():
    ...
    #TODO: Why do we use threads with batch size 1? seems slower than batching
...
class MegaMolBARTLatent(MegaMolBART):
    ...
    def interpolate_molecules(self, smiles1, smiles2, num_interp, tokenizer, k=1, sanitize=True):
        ...
        # add 2 for start / stop
        pad_length = max(len(smiles1), len(smiles2)) + 2
        embedding1, z1, z_mean1, z_logv1, pad_mask1 = self.smiles2embedding(smiles1, pad_length=pad_length)

        embedding2, z2, z_mean2, z_logv2, pad_mask2 = self.smiles2embedding(smiles2,pad_length=pad_length)
        ...
        # dims: batch, tokens, embedding
        # interpolated_emb = torch.lerp(embedding1, embedding2, scale).cuda()
        interpolated_emb = torch.lerp(z_mean1, z_mean2, scale).cuda()
        combined_mask = (pad_mask1 & pad_mask2).bool().cuda()

...
def rankdata_DEPRECATED(data, method='average', axis=None):
    """
    ...
    See also scipy.stats.rankdata, for which this function is a drop-in replacement
    """
    ...
    if axis is not None:
        # Replace with cupy.apply_along_axis when available in from dev
        if axis == 1:
            return cupy.array([rankdata(x, method=method) for x in arr])
        else:
            return cupy.array([rankdata(x, method=method) for x in arr.T]).T
    
    arr = cupy.ravel(arr)
    sorter = cupy.argsort(arr)

    inv = cupy.empty(sorter.size, dtype=cupy.intp)
    inv[sorter] = cupy.arange(sorter.size, dtype=cupy.intp)

    if method == 'ordinal':
        return inv + 1
    
    arr = arr[sorter]
    obs = cupy.r_[True, arr[1:] != arr[:-1]]
    dense = obs.cumsum()[inv]
    
    if method == 'dense':
        return dense

    count = cupy.r_[cupy.nonzero(obs)[0], len(obs)]
...
def spearmanr_DEPRECATED(data_matrix1, data_matrix2, top_k=100):
